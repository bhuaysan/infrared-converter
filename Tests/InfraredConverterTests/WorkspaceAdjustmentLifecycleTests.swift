...
struct WorkspaceAdjustmentLifecycleTests {
    ...
    func aRefusedRenderIsNotSaved() async throws {
        ...
        #expect(store.saved(for: Self.urlA)?.orientation == .quarterTurnRight)
        #expect(!state.hasPendingAdjustmentWork)
        #expect(store.writeSummary == ["lifecycle-a.orf:builtin.uncalibrated:rotate90Clockwise:identity:0.0EV:defaultNeutralPatch:0.0-1.0:0.0C"])
    }
    ...
    func aReopenedFileReportsTheDiskState() async throws {
        ...
        #expect(store.writeSummary == ["lifecycle-a.orf:builtin.uncalibrated:rotate180:identity:0.0EV:defaultNeutralPatch:0.0-1.0:0.0C"])
    }
    ...
    func anOlderGenerationCannotOverwriteANewerSave() async throws {
        ...
        #expect(store.writeSummary == [
            "lifecycle-a.orf:builtin.uncalibrated:rotate90Clockwise:identity:0.0EV:defaultNeutralPatch:0.0-1.0:0.0C", "lifecycle-a.orf:builtin.uncalibrated:rotate270Clockwise:identity:0.0EV:defaultNeutralPatch:0.0-1.0:0.0C"
        ])
    ...
    func aBurstAcrossASwitchPersistsOnce() async throws {
        ...
        #expect(store.writeSummary == ["lifecycle-a.orf:builtin.uncalibrated:\(settled.persistedToken):identity:0.0EV:defaultNeutralPatch:0.0-1.0:0.0C"])
