...
struct ExportImageEncoderTests {
    ...
    func provenanceIsCarriedThrough() throws {
        let exposed = try ExportTestData.toneCurved(
            width: 1, height: 1,
            values: [0.2, 0.4, 0.6],
            exposureEV: 1.5,
    ...
    func aReducedPreviewIsRefused() throws {
        let preview = try ExportTestData.toneCurvedFromPreview(
    ...
    func inconsistentGeometryIsRefused() {
        let broken = ToneCurvedRGBImage(
            width: 4,
            height: 4,
            values: [0, 0, 0],
            processing: DisplayPreviewTestData.contrastProcessing()
        )
